...
function obj_layout.PlaceWaterSetPieces(set_pieces, add_entity, checkFn)
    for name, data in pairs(set_pieces) do
        ...
        for i = 1, data.count or 1 do
            obj_layout.PlaceWaterLayout(layout, prefabs, add_entity, checkFn, data.radius or 0)
        end
    end
end

function obj_layout.PlaceSingleWaterSetPiece(name, add_entity, checkFn, radius)
    local layout = obj_layout.LayoutForDefinition(name)

    assert(layout, "could not find layout with " .. (name or "nil"))
    local prefabs = obj_layout.ConvertLayoutToEntitylist(layout)
    obj_layout.PlaceWaterLayout(layout, prefabs, add_entity, checkFn, radius)
end
